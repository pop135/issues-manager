...
if(!empty($_POST) && $_POST['Action']=='login_form'){
    ...
    $result = mysqli_query($con, "SELECT * FROM users WHERE email='$email' AND password='".md5($password)."' AND status='1' LIMIT 1");
...
if(!empty($_POST) && $_POST['Action']=='registration_form'){
    ...
    if(mysqli_num_rows($result)==1){
        ...
        mysqli_query($con, "INSERT INTO users (user_GUID, email, password, entry_date) values(MD5(UUID()), '$email', '".md5($password)."' ,NOW() )");
        ...
        mysqli_query($con, "INSERT INTO `user_profile` (user_id,name) VALUES('$user_id','$name')");
        /* Success: Set session variables and redirect to Protected page */
